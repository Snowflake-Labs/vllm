...
class GenericBatchExpansionTop1Scorer(SpeculativeScorer):
    ...
    def _expand_batch(
    ...
    ) -> Tuple[List[int], List[int], List[int], List[SequenceGroupMetadata], int]:
        ...
        #
        # Modified in https://github.com/snowflakedb/vllm/pull/178 to support
        # chunked-prefill. This is a temporary solution until we have a more
        # general solution in upstream vLLM:
        # https://docs.google.com/document/d/1I6H8bGYfwM8YappcP8HuoBhpS_pMlN0UIH2e_w9hv_M.
        #
        # For chunked-prefill, vLLM expects all prefill requests to be before
        # any decoding requests. Therefore, we order the requests as follows:
        # 1. Requests with is_prompt == True and do_sample = False.
        # 2. Requests with is_prompt == True and do_sample = True.
        # 3. Requests with proposal_len == 0 (no-prefill no-speculate).
        # 4. Requests with proposal_len > 0 (no-prefill speculate).
        prefill_indices = []
        non_spec_indices = []
        spec_indices = []
    ...
    def _contract_batch(
        ...
        k: int, prefill_indices: List[int],
        ...
        spec_expanded_bs = expanded_batch_size - non_spec_expanded_bs - len(prefill_indices)
    ...
    def _split_scoring_output(
        ...
        # done by supporting per-sequence proposal lens.
        #
        # First samples are from non speculative scoring, latter samples are from
        # speculative samples.
