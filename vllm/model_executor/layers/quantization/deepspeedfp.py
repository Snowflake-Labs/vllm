        ...
        def quant_weight_loader(param, loaded_weight, *args, **kwargs):
            if weight_loader is not None:
                if not hasattr(param, 'shadow_data'):
                    ...
                    param.loading_cont = (
                        loaded_weight.shape[0] // param.orig_shape[0] // tp_size
                        if loaded_weight.shape[0] != param.orig_shape[0] else
                        loaded_weight.shape[1] // param.orig_shape[1] // tp_size
                        if loaded_weight.shape[1] != param.orig_shape[1] else 1
                    )
                weight_loader(
                    param.shadow_data,
                    loaded_weight,
            ...
            if not hasattr(param, 'loading_cnt') or param.loading_cnt == 0:
                param.ds_quantize_(
                    loaded_weight.transpose(-1, -2).contiguous().cuda()
                    if self.enable_fused_kernel else loaded_weight.cuda()
                )
...
class DeepSpeedFPParameter(nn.Parameter):
    ...
    def __new__(cls, orig_shape: torch.Size, params_dtype: torch.dtype,
        ...
        g_size = max(
            2 ** i
            for i in range(4, int(math.log2(quant_config.group_size) + 1))
        ...
        self.fp_quantizer.scale = torch.empty(
            orig_shape.numel() // g_size,
            4,
            dtype=torch.uint8,
    ...
    def ds_dequantize(self, fp_out=None) -> torch.Tensor:
        ...
        """
        assert (
            self.data.device.type == "cuda" and
            self.data.dtype == torch.uint8
        )
