...
def fused_fp8_gemm(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        # Dequantize weight (fp8 -> bf16)
        dst_exponent = ((b & _exponent_mask) >> _mantisa_bits)
        sign = ((b & _sign_mask) >> (_mantisa_bits + _exponent_bits))
        dst_mantisa = (b & _mantisa_mask)
        b = ((sign << (q_exponent_bits + q_mantisa_bits)) | dst_mantisa << (q_mantisa_bits - _mantisa_bits))
        dst_exponent = (dst_exponent + _exp_bias)
        b = (b | (dst_exponent << q_mantisa_bits)).to(tl.uint16)
        b = (b.to(tl.bfloat16, bitcast=True) * scale).to(tl.bfloat16)
...
def invoke_fused_fp8_kernel(A: torch.Tensor, 
                            B: torch.Tensor, 
    ...
    grid = lambda META: (triton.cdiv(A.shape[0], META[
        'BLOCK_SIZE_M']) * triton.cdiv(B.shape[0], META['BLOCK_SIZE_N']), )
