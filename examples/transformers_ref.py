...
for i in range(0, 10):
    if i==0:
        q_len = 15
        kv_seq_len = q_len
        hidden_states = torch.zeros((bsz, q_len, emb_sz))
        attention_mask = torch.tril(torch.ones((bsz, 1, q_len, kv_seq_len)))  # ((bsz, 1, q_len, kv_seq_len):)
        position_ids = torch.arange(0, q_len)[None, :]
        hidden_states = torch.zeros((bsz, q_len, emb_sz))
    ...
    outputs = llama_pos_shift_attention_forward(
        layer,
        ...
        past_key_value=past_key_value,  # Optional[Tuple[torch.Tensor]] = None,
        output_attentions=output_attentions,    # bool = False,
        use_cache=use_cache,    # bool = False,
    )
    print("dupa")